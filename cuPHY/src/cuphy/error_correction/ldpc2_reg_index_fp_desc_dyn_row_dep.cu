...
namespace
{
    ...
    template <int BG_, int NUM_STORAGE_WORDS, class TKernelParams>
    struct ldpc2_reg_index_fp_desc_dyn_row_dep_kernel_config
    {
        ...
        template <int bg_> using bg_desc_t   = ldpc2::BG_desc<bg_>;
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG1_reg_index_fp_desc_dyn_row_dep()
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG1_reg_index_fp_desc_dyn_row_dep(LDPC_kernel_params params, app_loc_t<1>::bg_desc_t bgdesc)
{
    ...
    typedef ldpc2_reg_index_fp_desc_dyn_row_dep_kernel_config<1,
                                                              5,
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG2_reg_index_fp_desc_dyn_row_dep()
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG2_reg_index_fp_desc_dyn_row_dep(LDPC_kernel_params params, app_loc_t<2>::bg_desc_t bgdesc)
{
    // Shared memory is allocated dynamically
    extern __shared__ char smem[];
    ...
    //------------------------------------------------------------------
    // Kernel configuration template
    typedef ldpc2_reg_index_fp_desc_dyn_row_dep_kernel_config<2, // BG
                                                              5, // Num storage words
                                                              ldpc2::LDPC_kernel_params> kernel_config_t;
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG1_reg_index_fp_desc_dyn_row_dep_tb()
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG1_reg_index_fp_desc_dyn_row_dep_tb(cuphyLDPCDecodeDesc_t decodeDesc, app_loc_t<1>::bg_desc_t bgdesc)
{
    // Shared memory is allocated dynamically
    extern __shared__ char smem[];
...
////////////////////////////////////////////////////////////////////////
// ldpc2_BG2_reg_index_fp_desc_dyn_row_dep_tb()
extern "C"
__global__ __launch_bounds__(MAX_THREADS_PER_CTA, MIN_CTA_PER_SM)
void ldpc2_BG2_reg_index_fp_desc_dyn_row_dep_tb(cuphyLDPCDecodeDesc_t decodeDesc, app_loc_t<2>::bg_desc_t bgdesc)
{
    // Shared memory is allocated dynamically
    extern __shared__ char smem[];
...
////////////////////////////////////////////////////////////////////////
// reg_index_fp_desc_dyn_row_dep::decode()
cuphyStatus_t reg_index_fp_desc_dyn_row_dep::decode(ldpc::decoder&                     dec,
                                                    LDPC_output_t&                     tDst,
                                                    const_tensor_pair&                 tLLR,
                                                    const cuphyLDPCDecodeConfigDesc_t& config,
                                                    cudaStream_t                       strm)
{
    DEBUG_PRINTF("ldpc::decode_ldpc2_reg_index_fp_desc_dyn_row_dep()\n");
    //------------------------------------------------------------------
        ...
        case 1:
            {
                ...
                //------------------------------------------------------------------
                // Launch the kernel
                ldpc2_BG1_reg_index_fp_desc_dyn_row_dep<<<grdDim, blkDim, SHMEM_SIZE, strm>>>(params, *bgdesc);
        ...
        case 2:
            {
                ...
                DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG2_reg_index_fp_desc_dyn_row_dep, blkDim, SHMEM_SIZE);
                
                //------------------------------------------------------------------
                // Launch the kernel
                ldpc2_BG2_reg_index_fp_desc_dyn_row_dep<<<grdDim, blkDim, SHMEM_SIZE, strm>>>(params, *bgdesc);
                s = CUPHY_STATUS_SUCCESS;
            }
            break;
        default:
...
////////////////////////////////////////////////////////////////////////
// reg_index_fp_desc_dyn_row_dep::decode_tb()
cuphyStatus_t reg_index_fp_desc_dyn_row_dep::decode_tb(ldpc::decoder&               dec,
                                                       const cuphyLDPCDecodeDesc_t& decodeDesc,
                                                       cudaStream_t                 strm)
{
    DEBUG_PRINTF("ldpc::decode_tb_ldpc2_reg_index_fp_desc_dyn_row_dep()\n");
    
    cuphyStatus_t s = CUPHY_STATUS_NOT_SUPPORTED;

    if(decodeDesc.config.llr_type == CUPHY_R_16F)
    {
        // For now, we launch a CTA for each codeword. In the future, we
        // may want to loop over codewords from a single CTA.
        ...
        case 1:
            {
                ...
                DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG1_reg_index_fp_desc_dyn_row_dep_tb, blkDim, SHMEM_SIZE);

                //------------------------------------------------------------------
                // Launch the kernel
                ldpc2_BG1_reg_index_fp_desc_dyn_row_dep_tb<<<grdDim, blkDim, SHMEM_SIZE, strm>>>(decodeDesc, *bgdesc);
                s = CUPHY_STATUS_SUCCESS;
        ...
        case 2:
            {
                ...
                DEBUG_PRINT_FUNC_MAX_BLOCKS(ldpc2_BG2_reg_index_fp_desc_dyn_row_dep_tb, blkDim, SHMEM_SIZE);
                
                //------------------------------------------------------------------
                // Launch the kernel
                ldpc2_BG2_reg_index_fp_desc_dyn_row_dep_tb<<<grdDim, blkDim, SHMEM_SIZE, strm>>>(decodeDesc, *bgdesc);
                s = CUPHY_STATUS_SUCCESS;
            }
            break;
        default:
...
////////////////////////////////////////////////////////////////////////
// reg_index_fp_desc_dyn_row_dep::get_workspace_size()
std::pair<bool, size_t> reg_index_fp_desc_dyn_row_dep::get_workspace_size(const ldpc::decoder&               dec,
                                                                          const cuphyLDPCDecodeConfigDesc_t& config,
                                                                          int                                num_cw)
{
...
////////////////////////////////////////////////////////////////////////
// reg_index_fp_desc_dyn_row_dep::reg_index_fp_desc_dyn()
reg_index_fp_desc_dyn_row_dep::reg_index_fp_desc_dyn_row_dep(ldpc::decoder& desc)
{
    //------------------------------------------------------------------
    // Determine the maximum amount of shared memory
    ...
    const uint32_t MAX_BG2_SHMEM_SIZE = shmem_llr_buffer_size(ldpc2::max_variable_nodes<2>::value, // num shared memory nodes
                                                              ...
                                                              sizeof(__half));                     // element size
    //------------------------------------------------------------------
    // Set kernel function attributes (max dynamic shared memory)
    cudaError_t e = cudaFuncSetAttribute(ldpc2_BG1_reg_index_fp_desc_dyn_row_dep,
    ...
    e = cudaFuncSetAttribute(ldpc2_BG2_reg_index_fp_desc_dyn_row_dep,
                             cudaFuncAttributeMaxDynamicSharedMemorySize,
                             MAX_BG2_SHMEM_SIZE);
    if(cudaSuccess != e)
    {
    ...
    e = cudaFuncSetAttribute(ldpc2_BG1_reg_index_fp_desc_dyn_row_dep_tb,
                             cudaFuncAttributeMaxDynamicSharedMemorySize,
                             MAX_BG1_SHMEM_SIZE);
    if(cudaSuccess != e)
    {
    ...
    e = cudaFuncSetAttribute(ldpc2_BG2_reg_index_fp_desc_dyn_row_dep_tb,
                             cudaFuncAttributeMaxDynamicSharedMemorySize,
                             MAX_BG2_SHMEM_SIZE);
    if(cudaSuccess != e)
    {
    ...
    //------------------------------------------------------------------
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG1_reg_index_fp_desc_dyn_row_dep);
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG2_reg_index_fp_desc_dyn_row_dep);
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG1_reg_index_fp_desc_dyn_row_dep_tb);
    DEBUG_PRINT_FUNC_ATTRIBUTES(ldpc2_BG2_reg_index_fp_desc_dyn_row_dep_tb);
}

////////////////////////////////////////////////////////////////////////
// reg_index_fp_desc_dyn_row_dep::get_launch_config()
cuphyStatus_t reg_index_fp_desc_dyn_row_dep::get_launch_config(const ldpc::decoder&           dec,
                                                               cuphyLDPCDecodeLaunchConfig_t& launchConfig)
{
    ...
    cudaError_t    e = (BG == 1) ? cudaGetFuncBySymbol(&deviceFunction, (void*)ldpc2_BG1_reg_index_fp_desc_dyn_row_dep_tb) : 
                                   cudaGetFuncBySymbol(&deviceFunction, (void*)ldpc2_BG2_reg_index_fp_desc_dyn_row_dep_tb);
    if (e != cudaSuccess) 
    {
