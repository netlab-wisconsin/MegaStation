...
int main(int argc, char **argv) {
  ...
  std::string log_file = std::string(dir) + "reliable" + std::to_string(ants) +
  ...
  constexpr uint32_t kNumFrames = 10;
  constexpr uint32_t kNumRrus = 3;
  uint32_t gframe_id = 0;

  for (int round = 0; round < 210; round++) {
    bool start = false;
    std::vector<std::thread> producers;
    for (uint32_t rru_id = 0; rru_id < kNumRrus; rru_id++) {
      producers.push_back(
          std::thread([gframe_id, rru_id, &scheduler, &start]() {
            moodycamel::ProducerToken ptok(scheduler.task_queue);
            mega::TimerCPU timer_sleep;

            while (start == false) {
            }
            const uint32_t total_frames = kNumFrames + gframe_id;
            for (uint32_t rframe_id = gframe_id; rframe_id < total_frames;
                 rframe_id++) {
              const uint32_t frame_id = rframe_id * kNumRrus + rru_id;
              ...
              for (uint16_t symbol_id = 0; symbol_id < mega::gconfig.symbols;
                ...
                timer_sleep.stop();
                while (uint64_t(timer_sleep.get_duration_ms() * 1000.0) <
                       sleep_time)
                  timer_sleep.stop();
              }
            }
    ...
    my_logger->info("{},{}", gframe_id,
                    kNumFrames * kNumRrus / timer.get_duration_ms());
    gframe_id += kNumFrames;
  }
